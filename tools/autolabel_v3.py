# Auto-labeling v3: Guarantee 100% coverage
# Every image MUST have at least one detection.
#
# Strategy:
#   1. Multi-prompt inference (same as v2)
#   2. If no detection → retry with progressively lower thresholds
#   3. If still nothing → use full-image bbox as fallback
#   4. Enhanced prompts for previously low-coverage classes
...
PROMPT_MAP = {
    ...
    "empty":           ["empty tray", "empty plate", "empty pan", "empty container", "tray", "plate", "pan", "baking sheet"],
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='YOLO-World Auto-Labeling v3: 100% coverage guarantee')
    ...
    parser.add_argument('--output-dir',
                        default='data/labels_v3/experiments/ratio_real_only')
...
def main():
    ...
    print(f"\nFallback thresholds: {FALLBACK_THRESHOLDS}")
    print(f"Full-image fallback: {'OFF' if args.no_fullimg_fallback else 'ON'}")

    # Load model
    print(f"\nLoading model...")
    cfg = Config.fromfile(args.config)
    cfg.work_dir = osp.join('./work_dirs', 'autolabel_v3')
    ...
    print(f"\n{'='*60}")
    print(f"Auto-labeling v3 Complete!")
    print(f"{'='*60}")
